...
export default function ReportDetailPage() {
    ...
    return (
        <main className="min-h-screen bg-[#1A1B1E] p-6 lg:p-12 text-white">
        <div className="max-w-7xl mx-auto">
            
            {/* 뒤로가기 버튼 */}
            <button 
            onClick={() => window.history.back()}
            className="mb-6 inline-flex items-center gap-2 rounded-2xl bg-white/5 border border-white/10 px-5 py-3 text-sm font-bold text-white/70 hover:bg-white/10 hover:text-white transition-all active:scale-95"
            >
            <span>←</span> 뒤로가기
            </button>

            {/* --- 헤더 카드 섹션 --- */}
            <section className="mb-10 p-8 bg-[#25262B] rounded-[24px] border border-white/10 flex items-center justify-between shadow-2xl">
            <div className="flex items-center gap-8">
                {/* 기술 로고 아이콘 */}
                <div 
                className="w-20 h-20 rounded-2xl flex items-center justify-center border border-white/10 shadow-inner"
                ...
                <div className="flex items-center gap-4 mt-1">
                    <p className="text-white/60 text-sm">{techInfo.desc}</p> {/* 기술 종류 */}
                    ...
                    target="_blank" 
                    rel="noreferrer"
                    className="text-white/40 hover:text-blue-400 text-xs underline underline-offset-4 transition"
                    >
                    공식 사이트 방문하기 ↗
                    </a>
                </div>
            ...
            <button 
                ...
                className={`w-14 h-14 rounded-2xl border transition-all flex items-center justify-center group active:scale-95 ${
            ...
            >
                ...
                stroke={isTechStackFavorite(decodedId) ? "#EAB308" : "#9FA0A8"} 
                className={`w-6 h-6 transition-transform group-active:scale-90 ${!isTechStackFavorite(decodedId) && 'group-hover:stroke-white'}`}
                strokeWidth="2"
                >
